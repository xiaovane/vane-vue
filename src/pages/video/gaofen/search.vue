<template>
    <div id="search">
        <Header @transeferWord="search"></Header>
        <section class="container">
            ...
            <div class="paging" id="page1">
                <div v-for="item in foot.pages">
                    <a @click="click(item)" :class="{ 'red': change(item),'show':show(item)}">{{item}}</a>
                </div>
            </div>
        </section>
    </div>
    ...
    export default {
        ...
        methods: {
            search: function (data) {
                let that = this;
                let params=data==null?that.$route.query:data;
